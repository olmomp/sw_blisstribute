...
class soap_transport_http extends nusoap_base {
	...
	*/
	function setURL($url) {
		...
		// @todo BEGIN fix hack for PIXI fault - Host header!
		if (defined('CURL_FORGED_HOST_HEADER')) {

			$this->setHeader('Host', CURL_FORGED_HOST_HEADER);
		}
		// @todo END fix hack for PIXI fault - Host header!		
...
//		}
	...
	*/
	...
	function buildPayload($data, $cookie_str = '') {
	    ...
	    #echo "HTTP header: ".$this->outgoing_payload."\n";
	    #die();
	}
	...
	*/
	function sendRequest($data, $cookies = NULL) {
	  ...
	  if ($this->io_method() == 'socket') {
		...
		$curl_headers = array();
		foreach($this->outgoing_headers as $k => $v){
			if ($k == 'Connection' || $k == 'Content-Length' /* @todo removed by xris for pixi: || $k == 'Host'*/ || $k == 'Authorization' || $k == 'Proxy-Authorization') {
...
//		}
		...
		curl_setopt($this->ch, CURLOPT_TIMEOUT, 9000); 
